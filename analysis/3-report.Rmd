...
nodeRenderInfo(gR) <- list(fontsize=20)
renderGraph(gR)

tiff(width = 20, height = 8, units = "in", res = 300, file = "../manuscript/fig2.tiff")
renderGraph(gR)
dev.off()
...
sim <- cpdist(fit, nodes = c("ndi_4", "anx_2"), n = 10^4,
               evidence = (TRUE))
...
#### Influence of `anx_2` and `ndi_4` when `ses_3` and `neckarm_pain_2` are both constant
